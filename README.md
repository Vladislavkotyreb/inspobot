...
Кроме утренней рассылки бот умеет искать по запросу — командой `/pick`,
кнопками, в четыре нажатия:

1. **Что ищем** — экраны, флоу (многошаговые сценарии) или секции сайта;
2. **Платформа** — мобилка или десктоп (у секций шаг пропускается: они всегда веб);
3. **Аудитория** — B2B или B2C;
4. **Тема** — 8–10 вариантов, свои для каждого сочетания.

Первая, вторая и четвёртая оси опираются на настоящие параметры Mobbin
(`platform`, три разных инструмента поиска, текст запроса). Фильтра B2B/B2C
у Mobbin нет вовсе — это разделение задаётся формулировкой запроса: у B2B
спрашивается про биллинг и роли, у B2C — про пейволл и ленту. Бот честно
пишет об этом на самом шаге, чтобы не выглядело жёстким фильтром.

Состояние мастера лежит в `callback_data` кнопки, а не в базе: кнопки в старом
сообщении работают и после перезапуска бота, и нечего чистить.
...
| `/now` | собрать и прислать дневную подборку прямо сейчас |
...
91 тест: ротация тем, разбор и валидация ответа модели, форматирование под
лимиты Telegram, дедупликация в SQLite, форма запроса к Messages API,
самопроверка доступов с разбором частых отказов API, машина состояний мастера с кодированием кнопок и вся оркестровка с подменёнными Claude и Telegram.
